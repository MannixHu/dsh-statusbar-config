	...
	factory: (require) => {
		...
		*/
		function decodeSettings(value) {
			...
			if (record.template !== void 0 && typeof record.template !== "string") return void 0;
			return {
		...
		function StatusbarSettingsCard({ settings, t }) {
			...
			const reset = async () => {
				...
				setDraft(null);
				try {
					for (const key of ["enabled", "template"]) await settings.unset(key);
					setNotice({
						kind: "success",
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dsc-row dsc-template-row",
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										className: "dsc-chips",
										role: "group",
										"aria-label": t("card.chipsLabel"),
										children: VARIABLES.map((name) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											type: "button",
											className: "dsc-chip",
											"data-tip": t(`var.${name}`),
		...
		const ALL_SEGMENTS = Object.freeze({
			turns: true,
			steps: true,
			llmTime: true,
			toolTime: true,
		...
		function formatSecondsNumber(ms) {
			const seconds = ms / 1e3;
		...
		*/
		...
		function buildStatsGroups(stats, usage, segments, t) {
			const groups = [];
			if (stats !== void 0 && stats.steps > 0) {
				if (segments.turns && segments.steps) groups.push(t("stats.counts", {
					turns: stats.turns,
					steps: stats.steps
				}));
				...
				if (segments.throughput && stats.decodeMs > 0) speeds.push(t("stats.tokensPerSecond", { throughput: formatTokensPerSecond(stats.decodeTokens / (stats.decodeMs / 1e3)) }));
			...
			if (usage !== void 0 && (billedInputTokens(usage) > 0 || usage.outputTokens > 0)) {
				...
				if (segments.inputTokens && segments.outputTokens) groups.push(t("stats.tokens", {
					input,
					output
				}));
		...
		*/
		function renderTemplate(template, stats, usage) {
			const values = {};
			if (stats !== void 0) {
				...
				values.tps = stats.decodeMs > 0 ? formatTokensPerSecond(stats.decodeTokens / (stats.decodeMs / 1e3)) : "";
			}
			if (usage !== void 0) {
		...
		function ConfigurableStatsLine({ settings, t, useChat, useProjection }) {
			const snapshot = (0, react.useSyncExternalStore)((listener) => settings.subscribe(listener), () => settings.getSnapshot(), () => settings.getSnapshot());
			const settledNodes = useChat((s) => s.legacy.nodes);
			const usage = useProjection("tokenUsage");
			const projectedStats = useProjection("sessionStats");
			const stats = (0, react.useMemo)(() => projectedStats ?? deriveStats(settledNodes ?? []), [projectedStats, settledNodes]);
			const value = snapshot.value ?? DEFAULT_SETTINGS;
			const template = value.template.trim();
			const rendered = (0, react.useMemo)(() => template !== "" ? renderTemplate(template, stats, usage) : "", [
				template,
				stats,
				usage
			]);
			const hasData = (stats?.steps ?? 0) > 0 || usage !== void 0 && (billedInputTokens(usage) > 0 || usage.outputTokens > 0);
			const groups = !value.enabled || !hasData ? [] : template !== "" ? rendered.trim() !== "" ? [rendered.trim()] : [] : buildStatsGroups(stats, usage, ALL_SEGMENTS, t);
...
.dsc-chip::after{content:attr(data-tip);position:absolute;top:calc(100% + 6px);left:var(--tip-left,0px);transform:none;width:max-content;max-width:min(320px,80vw);box-sizing:border-box;background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:6px 10px;font-size:12px;line-height:1.5;white-space:normal;text-align:left;box-shadow:var(--dsw-shadow-lv1);opacity:0;pointer-events:none;transition:opacity .12s;z-index:5}
